...
当前可通过 Docker 构建含主页统计、标签页与节目导航，包含一篇演示和三篇真实精编的 VitePress 站点、运行 40 项测试、检查公开内容、采集五个 Feed 并查看剩余 16 条待处理项。M3 本批使用两份官方 transcript 和一次 Video Agent Kit ASR，三篇文章均经过完整源材料复核；私有逐字稿、音频和批次记录未进入 Git。远端工作流和部署状态只有实际运行后才会更新，私有备份恢复与用户阅读验收也必须由相应环境实际完成。
