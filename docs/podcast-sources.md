...
> 原则：只记录公开、无需私有令牌的 Feed；RSS 只是发现和元信息入口，Actions 不下载音频、不做 ASR、不调用大模型。本版同步 v0.7 的阅读目标、处理预算和英文文档命名；来源池地址与首批选择不变，未重新执行外部来源验证。
...
核查日期：2026-09-11（沿用 v0.6 的核查记录，不代表 v0.7 又执行了一次验证）。

v0.6 文档记录：上述来源仍可找到公开节目入口，主要 RSS 地址做过直接 Feed 端点检查；RSS/XML 端点不一定适合浏览器页面直接展示，但可供标准 RSS 解析器读取。v0.7 仅更新文档规则与命名，未重新核验上述结论。实际实施时仍以 `feedparser`/等价解析库的 smoke test 为最终准入条件。
