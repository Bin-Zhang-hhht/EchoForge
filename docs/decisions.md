...
- 首页使用 `doc` 布局纯 Markdown，首屏说明“读懂值得听的技术播客”，展示 5 篇最近整理，并提供摘要、节目、阅读时长和标签；首页不展示待处理数量或采集运行时间。
...
- 阅读时长使用 `阅读约 N 分钟`，由 `check.py` 按正文非空白字符和每分钟 400 字确定性复核；不再使用“全文 N 字 · 预计阅读 M 分钟”的公开格式。
- 模型信息只在文末“整理说明”中展示；文章页、首页、列表页、标签页和节目页关闭 VitePress 默认上一篇/下一篇导航。
- 标签遵循复用优先和单篇 2～4 个的软约束；`RecSys` 已归并到“推荐系统”。生成器当前不负责自动删除历史标签目录，清理旧派生产物属于构建维护步骤。
- `data/collected-at.json` 仍由采集器记录运行时间并随采集提交，但当前站点不读取或公开该字段。
