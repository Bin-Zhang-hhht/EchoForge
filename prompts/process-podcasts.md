...
3. Follow the frontmatter and meta blockquote in `templates/post.md`: `published_at` is the episode publish date from the item metadata (omit the field only when the item has no publish date); `transcribed_at` is the transcript retrieval date recorded in the private archive manifest; `model` records the model that actually produced this digest. Directly under the H1, write the four-line blockquote (`节目发布/逐字稿获取/笔记整理` dates, `全文 N 字 · 预计阅读 M 分钟 · 处理模型`, `标签` links, AI disclaimer) with a trailing backslash hard break on every line except the last, where N is the body character count as defined by the deterministic checker and M = max(1, ceil(N/400)). The `标签` line lists the frontmatter tags in order, each linked to `/tags/<tag>/` (percent-encode spaces as `%20`). `check.py` recomputes the counts and validates the links, so finalize the body before filling them in.
