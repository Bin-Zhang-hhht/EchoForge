...
3. Follow the frontmatter and meta blockquote in `templates/post.md`: `published_at` is the episode publish date from the item metadata (omit the field only when the item has no publish date); `transcribed_at` is the transcript retrieval date recorded in the private archive manifest; `model` records the model that actually produced this digest. Directly under the H1, write the blockquote as four separate lines inside one blockquote (`节目发布/逐字稿获取/笔记整理` dates, `全文 N 字 · 预计阅读 M 分钟`, `标签` links, and a closing `处理模型：<model> · AI 编辑整理，请以原始节目为准。` line) separated by blank `>` lines — no trailing backslash hard breaks — where N is the body character count as defined by the deterministic checker and M = max(1, ceil(N/400)). The `标签` line lists the frontmatter tags in order, each linked to `/tags/<tag>/` (percent-encode spaces as `%20`) and separated by single spaces without `·` separators; the site stylesheet renders that line as tag pills. `check.py` recomputes the counts and validates the links, so finalize the body before filling them in.
